    ...
    if (Helper::is_post()) {
        ...
        if(empty($_POST['id'])){
            ...
            $product->purchase_price = trim($_POST['purchase_price']);
             $product->reward = trim($_POST['reward']);
            ...
            $product->price_userA = trim($_POST['price_userA']);
            $product->price_userB = trim($_POST['price_userB']);
            $product->price_userC = trim($_POST['price_userC']);
            $product->description = trim($_POST['description']);
            $product->part_code = trim($_POST['part_code']);
            $product->hsn_code = trim($_POST['hsn_code']);
            $product->product_weight = trim($_POST['product_weight']);
           ...
           $priceForQuantity=[];
            ...
            foreach($_POST as $post_item => $qty){
                if(strpos($post_item, "qty") !== false){
                 ...
                 if(strpos($post_item, "prcQty") !== false){
                    $post_item = str_replace("prcQty", "", $post_item);
                    $post_item = str_replace("_", ",", $post_item);
                    array_push($priceForQuantity,$qty);
                    // if(empty($qty)) $has_qty_error = true;
                    // else{
                    //     $inventory[$post_item] = $qty;
                    // }
                    ...
                    if($id){
                        if(!empty($inventory)){
                            ...
                            foreach ($inventory as $key => $value){
                                ...
                                $inventory->price=$priceForQuantity[$iForPrcQtySingle];
                                $iForPrcQtySingle += 1;
                    ...
                    if($product->where(["id"=>$product->id])->andWhere(["admin_id" => $product->admin_id])->update()){
                        ...
                        if(!empty($inventory)){
                            ...
                            foreach ($inventory as $key => $value){
                                ...
                                $inventory->quantity = $value;
                                $inventory->price = $priceForQuantity[$iForPrcQty];
                                $iForPrcQty +=1;
                                if(!$inventory->save()){
                                    $has_error_creation = true;
                                    $errors->add_error("Something went wrong. Please Try Again.");
